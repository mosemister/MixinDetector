...
public class ScanningScreen extends JPanel {
    ...
    private final LinkedBlockingQueue<MixinFile> files = new LinkedBlockingQueue<>();
    ...
    private void run(File[] mods) {
        Stream.of(mods).forEach(modFile -> {
            try {
                ...
                JOptionPane.showInternalMessageDialog(null, e.getMessage(), Arrays
                        .stream(e.getStackTrace())
                        .map(element -> element.getLineNumber() + " : " + element.getClassName() + " : "
